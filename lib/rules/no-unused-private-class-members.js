	...
	create(context) {
		...
		function getLineStartIndex(nodeOrToken) {
			return sourceCode.getIndexFromLoc({
				line: nodeOrToken.loc.start.line,
				column: 0,
			});
		}
		...
		/**
		 * Gets the previous field token that needs a semicolon when removing a class member.
		 * @param {ASTNode} classMemberNode The member that would be removed
		 * @returns {Token|null} The previous member's last token, or null if no semicolon is needed
		 */
		function getSemicolonInsertionToken(classMemberNode) {
			const members = classMemberNode.parent.body;
			const memberIndex = members.indexOf(classMemberNode);
			...
			if (
				!previousMember ||
				!nextMember ||
				previousMember.type !== "PropertyDefinition" ||
				sourceCode.getLastToken(previousMember).value === ";"
			) {
				return null;
			}

			const nextMemberFirstToken = sourceCode.getFirstToken(nextMember);

			if (
				nextMemberFirstToken.value === "[" ||
				nextMemberFirstToken.value === "*"
			) {
				return sourceCode.getLastToken(previousMember);
			}
